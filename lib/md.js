...
function inline(raw) {
  ...
  s = s.replace(/(^|[^*])\*([^*\n]+)\*/g, '$1<em>$2</em>')
  s = s.replace(/(^|[^_])_([^_\n]+)_(?!\w)/g, '$1<em>$2</em>')
  s = s.replace(/\[([^\]]+)\]\(\u0000(\d+)\u0000\)/g, (_, t, i) => `<a href="${stash[Number(i)]}" target="_blank" rel="noopener noreferrer">${t}</a>`)
  s = s.replace(/\u0000(\d+)\u0000/g, (_, i) => {
    const v = stash[Number(i)]
    return v.startsWith('<code>') ? v : `<a href="${v}" target="_blank" rel="noopener noreferrer">${v}</a>`
  })
  return s
}
