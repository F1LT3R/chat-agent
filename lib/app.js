...
import { applyWidth, initResizeHandler, initTheme, initWidth } from './theme.js'

const $ = (s) => document.querySelector(s)
const UUID_RE = /^[0-9a-f-]{36}$/i
const APP_NAME = '🕵️ ChatAgent'
...
function upsertMeta(m) {
  ...
  if (state.current === meta.id) {
    ...
    setThinkBtn(meta.thinking === true)
  }
}

/* ---------------- routing (browser history) ---------------- */

async function openConversation(id, push, hash = '') {
  ...
  const box = $('#chat-col') // inner column; #messages stays the full-width scroll container
  box.innerHTML = ''
  for (const it of state.items) box.append(buildMessage(it))
  $('#chat-title').textContent = data.meta.title
  document.title = `${data.meta.title} · ${APP_NAME}`
  renderStats(data.meta)
  setThinkBtn(data.meta.thinking === true)
  // Deep link: a #heading in the URL (the page's own hash when arriving via
  // history, or one carried by a clicked [local chat link]) scrolls the
  // anchor into view instead of snapping to the bottom.
  const targetHash = push ? hash || '' : location.hash || ''
  if (push) history.pushState({}, '', `/chat/${id}${targetHash}`)
  closePanel()
  if (targetHash) {
    let el = document.getElementById(targetHash.slice(1))
    if (!el) {
      // Agent-built links carry the bare heading slug (no message-id
      // prefix); resolve it to the first heading whose scoped id ends with
      // that slug, then canonicalize the hash to the real id.
      const bare = targetHash.slice(1)
      el = [...document.querySelectorAll('#messages h1[id], #messages h2[id], #messages h3[id], #messages h4[id], #messages h5[id], #messages h6[id]')]
        .find((h) => h.id === bare || h.id.endsWith('-' + bare)) || null
      if (el) history.replaceState({}, '', `${location.pathname}#${el.id}`)
    }
    if (el) {
      // instant: smooth scroll never runs in an occluded/background tab
      // (e.g. opening a notification deep link), and a deterministic
      // alignment is what a shared link should give.
...
function setSendMode() {
  ...
  const label = state.turnActive ? 'Stop generating' : 'Send message'
  b.title = label
  // The glyph changes with mode; the accessible name must follow it.
...
function setPanelOpen(open) {
  ...
  // Focus management for the overlay drawer: on open, move focus into the
  // panel; on close, return focus to the toggle if it was inside the panel
  // (keyboard users must not be stranded in the closed drawer).
  const panel = $('#panel')
  if (open && !wasOpen && isNarrow()) {
    panel.focus({ preventScroll: true })
  } else if (!open && wasOpen && isNarrow() && document.activeElement?.closest('#panel')) {
    toggle.focus({ preventScroll: true })
  }
  // The in-flow menu width changes #messages' content width, so re-size and
  // re-center the column (screen-centered) whenever the panel shows/hides.
  if (open !== wasOpen) applyWidth()
}
...
function bindUi() {
  ...
  // Escape closes the overlay drawer (narrow viewports).
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape' && state.panelOpen && isNarrow()) setPanelOpen(false)
  })
  ...
  $('#think-btn').addEventListener('click', async () => {
    if (!state.current) return
    const cur = state.metas.get(state.current)?.thinking === true
  ...
  $('#messages').addEventListener('click', (e) => {
    const local = e.target.closest('a[data-local]')
    if (local) {
      // /chat/<uuid>[#heading] deep link: switch conversations in-app, the
      // hash is carried over so openConversation scrolls the anchor into
      // view.
      e.preventDefault()
      const href = local.getAttribute('href') || ''
      const m = href.match(/\/chat\/([0-9a-f-]{36})/i)
      if (m) openConversation(m[1], true, href.includes('#') ? href.slice(href.indexOf('#')) : '')
      return
    }
  ...
  // conversation list: open / rename (items are real buttons; the rename
  // button is handled first so it wins over the row click)
