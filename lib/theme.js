...
// #messages itself stays full-width so its scrollbar is pinned to the
// right edge of the screen; only the inner column + composer narrow.
export function applyWidth() {
	const pct = Math.round(widthRatio * 100) + '%'
	const col = document.querySelector('#chat-col')
	if (col) col.style.width = pct
	const composer = document.querySelector('#composer')
	if (composer) composer.style.width = pct
}
