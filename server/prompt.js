...
export function buildSystemPrompt() {
  ...
  if (!cached || Date.now() - cachedAt > 60000) {
    ...
    const sessions = loadSkill('SESSIONS.md')
    const parts = [
      ...
      `You can look into the operator's past chat sessions with the \`sessions\` tool; link back to them as markdown links to \`/chat/<id>\` paths so they are clickable.`,
    ...
    if (sessions) parts.push(`# Your past-sessions skill\n\n${sessions}`)
    cached = parts.join('\n\n')
    cachedAt = Date.now()
  }
  return cached
